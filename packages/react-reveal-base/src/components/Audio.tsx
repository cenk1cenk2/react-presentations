...
export type AudioTypes = '3gp' | 'aac' | 'flac' | 'mpg' | 'mpeg' | 'mp3' | 'mp4' | 'm4a' | 'oga' | 'ogg' | 'wav' | 'webm'
  ...
  if (Array.isArray(src)) {
    return (
      <audio {...getClassNameProps(props)} data-autoplay={autoplay} controls={controls} muted={muted} loop={loop} data-fragment-index={fragmentIndex}>
        {src.map((element) => (
          <source src={lazy ? false : element} data-src={lazy ? element : false} data-preload={preload} type={formatMap[/\.[^.]+$/.exec(element)?.[0] as AudioTypes] || 'mp3'} />
        ))}
      </audio>
    )
  }
