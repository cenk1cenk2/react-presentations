...
type VideoTypes = '3gp' | 'mpg' | 'mpeg' | 'mp4' | 'm4a' | 'm4p' | 'ogv' | 'ogg' | 'mov' | 'webm'
...
function Video ({ autoAnimateId, autoplay, controls, fragmentIndex, height, lazy, loop, muted, preload, width, src, ...props }: MakeProps<VideoProps, 'video'>) {
  if (Array.isArray(src)) {
    return (
      ...
      >
        {src.map((element) => (
          <source src={lazy ? '' : element} data-src={lazy ? element : false} data-preload={preload} type={formatMap[(/\.[^.]+$/.exec(element)?.[0] as VideoTypes) || 'mp4']} />
        ))}
      </video>
    )
  }
