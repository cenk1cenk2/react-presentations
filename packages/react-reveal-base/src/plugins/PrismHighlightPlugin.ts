...
import { PrismLanguages, PrismPlugins, PrismThemes } from './PrismHighlightPlugin/types'
...
export interface PrismOptions<CustomLanguages extends string = PrismLanguages, CustomPlugins extends string = PrismPlugins, CustomThemes extends string = PrismThemes> {
  prism?: {
...
> = () => RevealPluginDefinition<PrismOptions<CustomLanguages, CustomPlugins, CustomThemes>>

export interface ActualPrism {
  languages: Prism.Languages
}

export function addCustomPrismLanguage (languageDef: (prism: ActualPrism) => void) {
  languageDef(Prism)
}
...
function scrollToTopmostHighlightedLine (currentSlide: Element) {
  ...
  for (const code of codeSections) {
    ...
    highlight.parentElement.scrollTo({
      top: highlight.offsetTop - (highlight?.parentElement?.parentElement?.scrollTop || 0) - 30,
      behavior: 'smooth'
    })
  }
}
