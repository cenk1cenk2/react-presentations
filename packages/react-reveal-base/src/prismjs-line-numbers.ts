export default function startup (Prism: any) {
  ...
  function resizeElements (elements: HTMLElement[]) {
    ...
    infos.forEach((info) => {
      const lineNumberSizer = info.sizer
      const wrapper = info.element.querySelector<HTMLElement>('.line-numbers-rows')
      ...
      info.lineHeights.forEach((height, lineNumber) => {
        ...
        (wrapper.children[lineNumber] as HTMLElement).style.height = `${height}px`
      })
    })
  }
   ...
   */
  const getStyles = function (element: HTMLElement) {
    if (!element) {
      return null
    }

    return window.getComputedStyle?.(element) || (element as any).currentStyle || null
  }
